...
static inline HMM_Mat4 HMM_Rotate_RPY(HMM_Vec3 Rpy){
    const float roll = HMM_AngleDeg(Rpy.X);
    const float pitch = HMM_AngleDeg(Rpy.Y);
    const float yaw = HMM_AngleDeg(Rpy.Z);

    const float su = HMM_SinF(roll);
    const float cu = HMM_CosF(roll);
    const float sv = HMM_SinF(pitch);
    const float cv = HMM_CosF(pitch);
    const float sw = HMM_SinF(yaw);
    const float cw = HMM_CosF(yaw);

    HMM_Mat4 Result = HMM_M4D(1.0f);

    Result.Elements[0][0] = cv*cw;
    Result.Elements[0][1] = su*sv*cw - cu*sw;
    Result.Elements[0][2] = su*sw + cu*sv*cw;
    Result.Elements[1][0] = cv*sw;
    Result.Elements[1][1] = cu*cw + su*sv*sw;
    Result.Elements[1][2] = cu*sv*sw - su*cw;
    Result.Elements[2][0] = -sv;
    Result.Elements[2][1] = su*cv;
    Result.Elements[2][2] = cu*cv;
